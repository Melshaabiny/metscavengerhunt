...
class user_authTest(unittest.TestCase):
    ...
    def tearDown(self):
        """
        **teatDown** method removes all the changes that the test 
        cases made for our system. This will be executed each time 
        that the test case runs.
        """
...
###############Test Register
    ...
    def test_register_status_code(self):
        """
        **test_register_status_code()** checks if a user access to 
        the register page, the status_code is 200.
        """
        # create client.
        ...
        **test_RegisterForm()** checks when request.method to **user_auth.views.register()** is
        ...
        with patch('user_auth.views.RegisterForm') as reg:
            reg.return_value = MagicMock() # only checking if the RegisterForm is called with
                                      # proper data.
            # this makes form.is_valid() returns False in views.py.                          
            reg.return_value.is_valid.return_value = False 
    ...
    def test_register_notUnique_username(self):
        """
        **test_register_notUnique_username()** tests if a user trying to 
        register with an username that is already exists in the database. 
        This test should be failed since our website not allow two users 
        with same username.
        """
...
###############Test login_user
    ...
    def test_LogInForm_get(self):
        """
        **test_loginForm()** checks if the LogInForm is instantiated 
        when an user access to login page. This test should be considered 
        as different test with **test_login_status_code()** since this 
        function just checks if the httpresponse returns the status_code 200. 
        """
    ...
    def test_LogInForm_post(self):
        """
        **test_LogInForm_post()** tests if the LogINForm is 
        properly instantiated with POST data. If the post data 
        looks like {'user_name':'test_user', 'password':'password'}, 
        then the LogInForm must be called with the argument 
        **QueryDict('user_name=test_user&password=password')**.
        """
    ...
    def test_login_user_invalid_username(self):
        """
        **test_login_user_invalid_username()** tests if an user 
        tries to login with invalid username such as non-exist username.
        """
...
###############Test login_user
    ...
    def test_edit_status_code(self):
        """
        **test_edit_status_code()** checks if a user access to the 
        edit page, the status_code equals to 200.
        """
